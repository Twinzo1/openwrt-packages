...
binpath=$(uci get AdGuardHome1.AdGuardHome.binpath)
if [ -z "$binpath" ]; then
uci set AdGuardHome1.AdGuardHome.binpath="/tmp/AdGuardHome/AdGuardHome"
binpath="/tmp/AdGuardHome1/AdGuardHome"
fi
mkdir -p ${binpath%/*}
upxflag=$(uci get AdGuardHome1.AdGuardHome.upxflag 2>/dev/null)

check_if_already_running(){
	running_tasks="$(ps |grep "AdGuardHome1" |grep "update_core" |grep -v "grep" |awk '{print $1}' |wc -l)"
...
check_latest_version(){
	check_wgetcurl
	control_ver=`uci get AdGuardHome1.AdGuardHome.cversion 2>/dev/null`
		...
		doupdate_core
			...
			if [ ! -z "$upxflag" ]; then
				...
				if [ $filesize -gt 8000000 ]; then
					...
					doupx
					mkdir -p "/tmp/AdGuardHome1update/AdGuardHome" >/dev/null 2>&1
					rm -fr /tmp/AdGuardHome1update/AdGuardHome/${binpath##*/}
					/tmp/upx-${upx_latest_ver}-${Arch}_linux/upx $upxflag $binpath -o /tmp/AdGuardHome1update/AdGuardHome/${binpath##*/}
					rm -rf /tmp/upx-${upx_latest_ver}-${Arch}_linux
					/etc/init.d/AdGuardHome1 stop nobackup
					rm $binpath
					mv -f /tmp/AdGuardHome1update/AdGuardHome/${binpath##*/} $binpath
					/etc/init.d/AdGuardHome1 start
					echo -e "finished"
				fi
			fi
			EXIT 0
...
doupdate_core(){
	...
	mkdir -p "/tmp/AdGuardHome1update"
	rm -rf /tmp/AdGuardHome1update/* >/dev/null 2>&1
	Archt="$(opkg info kernel | grep Architecture | awk -F "[ _]" '{print($2)}')"
	case $Archt in
	"i386")
	Arch="386"
	...
	*)
	...
	grep -v "^#" /usr/share/AdGuardHome1/links.txt >/tmp/run/AdHlinks.txt
	while read link
	do
		eval link="$link"
		$downloader /tmp/AdGuardHome1update/${link##*/} "$link" 2>&1
		if [ "$?" != "0" ]; then
			echo "download failed try another download"
			rm -f /tmp/AdGuardHome1update/${link##*/}
		else
			local success="1"
			break
		fi 
	...
	if [ "${link##*.}" == "gz" ]; then
		tar -zxf "/tmp/AdGuardHome1update/${link##*/}" -C "/tmp/AdGuardHome1update/"
		if [ ! -e "/tmp/AdGuardHome1update/AdGuardHome" ]; then
			echo -e "Failed to download core." 
			rm -rf "/tmp/AdGuardHome1update" >/dev/null 2>&1
			EXIT 1
		fi
		downloadbin="/tmp/AdGuardHome1update/AdGuardHome/AdGuardHome"
	else
		downloadbin="/tmp/AdGuardHome1update/${link##*/}"
	fi
...
EXIT(){
	rm /var/run/update_core1 2>/dev/null
	[ "$1" != "0" ] && touch /var/run/update_core1_error
	exit $1
}
main(){
	...
	touch /var/run/update_core1
	rm /var/run/update_core1_error 2>/dev/null
	main $1
