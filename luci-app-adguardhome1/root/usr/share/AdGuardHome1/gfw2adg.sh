...
nowmd5=${nowmd5%% *}
local lastmd5=$(uci get AdGuardHome1.AdGuardHome.gfwlistmd5 2>/dev/null)
if [ "$nowmd5" != "$lastmd5" ]; then
	uci set AdGuardHome1.AdGuardHome.gfwlistmd5="$nowmd5"
	uci commit AdGuardHome1
	[ "$1" == "noreload" ] || /etc/init.d/AdGuardHome1 reload
fi
}
configpath=$(uci get AdGuardHome1.AdGuardHome.configpath 2>/dev/null)
[ "$1" == "del" ] && sed -i '/programaddstart/,/programaddend/d' $configpath && checkmd5 "$2" && exit 0
gfwupstream=$(uci get AdGuardHome1.AdGuardHome.gfwupstream 2>/dev/null)
